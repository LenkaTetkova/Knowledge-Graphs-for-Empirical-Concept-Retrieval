...
def visualise_concepts_UMAP(X, cfg, concepts, concept_names, model_name, layer, path):
    reducer = umap.UMAP()
    ...
    plt.savefig(f"{path}{model_name}_{layer}_UMAP.png", dpi=cfg.dpi)
    plt.close()
    if "0" in str(layer):
        nice_names = concept_names
        fig = make_legend(cfg, nice_names, colors)
        fig.savefig(f"{path}{model_name}_UMAP_legend.png", bbox_inches="tight", dpi=500, pad_inches=0)
        # fig.close()
    return


@hydra.main(config_path="../config", config_name="default.yaml")
def plot_UMAP(cfg: DictConfig) -> None:
    ...
    for layer in cfg.layers:
        ...
        visualise_concepts_UMAP(latent_data, cfg, indices, concepts, cfg.model_name, layer, path_figures)
