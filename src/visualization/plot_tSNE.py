...
def make_legend(cfg, labels, col):
    fig, ax = plt.subplots(figsize=(cfg.fig_width * 0.5, cfg.fig_width * 2))
    for c, label in zip(col, labels):
        alpha = 0.4 if label == "background" else 1
        ax.scatter([], [], color=c, label=label.split(",")[0], marker="o", linewidths=0, edgecolors="none", alpha=alpha)

    plt.subplots_adjust(top=1.05, bottom=-0.05, right=1.05, left=-0.05, hspace=0, wspace=0)
    plt.margins(0, 0)
    ax.axis("off")
    ax.legend(loc="center", frameon=False, fontsize=cfg.font_size)
    return fig


def visualise_concepts_tSNE(X, cfg, concepts, concept_names, model_name, layer, path):
    ...
    plt.savefig(f"{path}{model_name}_{layer}_tSNE.png", dpi=cfg.dpi)
    plt.close()
    if "0" in str(layer):
        nice_names = concept_names
        fig = make_legend(cfg, nice_names, colors)
        fig.savefig(f"{path}{model_name}_tSNE_legend.png", bbox_inches="tight", dpi=500, pad_inches=0)
    return


@hydra.main(config_path="../config", config_name="default.yaml")
def plot_tSNE(cfg: DictConfig) -> None:
    ...
    for layer in cfg.layers:
        ...
        visualise_concepts_tSNE(latent_data, cfg, indices, concepts, cfg.model_name, layer, path_figures)
