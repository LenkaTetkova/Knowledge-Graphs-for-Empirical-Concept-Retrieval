# From https://github.com/JonathanCrabbe/CARs/blob/main/explanations/concept.py
import abc
import logging
import pickle
from abc import ABC
...
class ConceptExplainer(ABC):
    ...
    @abc.abstractmethod
    def evaluate(self, latent_reps: np.ndarray, labels: np.ndarray) -> float:
        """
    ...
    def save(self, path):
        ...
        """
        raise NotImplementedError  # TODO!  save(self.classifier, path)
    ...
    def load(self, path):
        ...
        """
        raise NotImplementedError  # TODO!  load(self.classifier, path)
...
class CAR(ConceptExplainer, ABC):
    ...
    def evaluate(self, latent_reps: np.ndarray, labels: np.ndarray) -> float:
        ...
        Returns:
            accuracy of the concept for the latent representations between 0 and 1
        """
        predictions = self.predict(latent_reps)
    ...
    def get_kernel_function(self) -> callable:
        ...
        if self.kernel == "rbf":
            ...
            return lambda h1, h2: torch.exp(
                -torch.sum(
                    ((h1.unsqueeze(1) - h2.unsqueeze(0)) / (latent_dim * kernel_width)) ** 2,
                    dim=-1,
                )
            )
